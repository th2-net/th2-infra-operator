...
class StringsTests {
    ...
    @Test
    void anyPrefixMatchTest() {
        List<String> prefixes = List.of("a", "b", "c", "D");
        String namespace = "dev-someone";
        assertFalse(Strings.anyPrefixMatch(namespace, prefixes));
        prefixes = List.of("dev", "not-dev");
        assertTrue(Strings.anyPrefixMatch(namespace, prefixes));
        assertFalse(Strings.anyPrefixMatch(null, null));
    }
